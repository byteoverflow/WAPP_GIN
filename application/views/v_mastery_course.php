								...
								for($x=1; $x<=intval($accessLevel); $x++)
								{
									...
									foreach(glob("$mediaDir/*.mp3") as $file) 
									{
										...
										$downloadFile = $mediaUrl."/".$filename;	
										 ...
										 echo '<br>
										 <a href="#" onclick="window.open('."'audioPlayer/?audioFile=".urldecode($downloadFile)."', 'AudioPlayer', 'width=350,height=250,location=no,scrollbars=no,status=no,toolbar=no,resizable=no'".' )"><img src="'.$playButton.'"></a>
